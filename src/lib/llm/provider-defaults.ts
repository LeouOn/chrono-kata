 ...
 * the app. DeepSeek, OpenRouter Flash, and ZAI presets verified 2026-09-21.
 * Users can override models in the LLM settings UI.
 *
...
export const PROVIDER_DEFAULTS = {
  ...
  zai:            { baseUrl: 'https://api.z.ai/api/coding/paas/v4',                model: 'glm-5.3' },
...
/** Upgrade exact retired presets without replacing custom URLs or model choices. */
export function refreshLegacyPreset(name: string, config: { baseUrl: string; model: string }) {
  let { baseUrl, model } = config;
  if (name === 'zai') {
    if (baseUrl.replace(/\/$/, '') === 'https://open.bigmodel.cn/api/paas/v4') baseUrl = PROVIDER_DEFAULTS.zai.baseUrl;
    if (model === 'glm-5.2') model = PROVIDER_DEFAULTS.zai.model;
  }
  if (name === 'deepseek' && model === 'deepseek-v4-flash') model = PROVIDER_DEFAULTS.deepseek.model;
  if (name === 'openrouter' && ['anthropic/claude-sonnet-latest', 'anthropic/claude-sonnet-4'].includes(model)) model = PROVIDER_DEFAULTS.openrouter.model;
